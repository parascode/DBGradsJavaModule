...
```sh
Vinsys@DESKTOP-KGC1AKC MINGW64 ~/Desktop
...
```sh
Vinsys@DESKTOP-KGC1AKC MINGW64 ~
\$ cd Desktop/
```
...
```sh
Vinsys@DESKTOP-KGC1AKC MINGW64 ~/Desktop/30-days-of-react (master)
\$ npm start
```
...
Create images folder in the src folder and save an image inside. For instance let's save Vinsys.jpg image and let's import this image to index.js. After importing we will inject it to a JSX expression, user. See the code below.

```js
// index.js
import React from 'react'
import ReactDOM from 'react-dom'
import VinsysImage from './images/Vinsys.jpg'

const user = (
  <div>
    <img src={VinsysImage} alt='Vinsys image' />
  </div>
)
